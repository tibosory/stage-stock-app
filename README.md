...
- Le **source** de l’installateur (backend StageStock, scripts Inno) se trouve en général dans le monorepo *stagestock* ; il faut **joindre** l’EXE généré (`Build-Installer.ps1`) aux releases **ici** quand vous publiez une version, pour que le lien ci-dessus reste valide.
- Côté build : `expo.extra.installerGitHubRepo` = `tibosory/stagestock` et `EXPO_PUBLIC_INSTALLER_GITHUB_REPO` (EAS, `eas.json`) — aligné avec l’exemple.
